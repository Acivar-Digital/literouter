...
export function lingResponseToSseStream(resp: OpenAIResponse): ReadableStream<Uint8Array> {
  const encoder = new TextEncoder();
  const choice = resp.choices?.[0];
  const streamId = resp.id || `chatcmpl_${Date.now()}`;
  const streamModel = resp.model || "ling-3.0-flash";
  const streamCreated = resp.created || Math.floor(Date.now() / 1000);
  ...
  return new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      // 4. Tool calls deltas (Chunk 1: declaration, Chunk 2: arguments)
      ...
      // 5. Finish chunk
      const finishReason = choice?.finish_reason || (toolCalls && toolCalls.length > 0 ? "tool_calls" : "stop");
      ...
      if (resp.usage) finishPayload.usage = resp.usage;
      controller.enqueue(encoder.encode(sse(finishPayload)));
