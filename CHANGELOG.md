...
- **Core Probe Tool Merging & `tool_choice` Normalization for Zen (`zn`)** (`literouter-4m2am`, `literouter-3k33x`):
  ...
  - **`tool_choice` Normalization**: Automatically converts `tool_choice: "none"` to `"auto"` across Chat Completions and Responses wires, preventing upstream Zen from rejecting requests where clients disable tool execution during summarization.
...
- **Analytics session count stdout-parquet alignment (`literouter-72vd`)**: `report.py` session count rule now applies `distinct non-null session_id + 1` when any null-session traces exist (matching `transform.session_rollups` parquet group height); stdout `Sessions:2` verified against parquet height. Final report `REPORT_20260917_165055.md` produced (Sessions:2, Traces:10, 30-day trend 10 traces at $0.02). 58 pytest green (`test_analytics.py`).
- **QA-fix Batch1+Batch2 (literouter-93pp,479w,xm9m,6ptn,mhat)**: Consolidated provider/429/user/api-key/depth/BYOK sections (§9.2/§9.5), turn-depth buckets, BYOK derived from directive key (52 $0 verdict YES). 74 pytest green (`test_analytics.py`); 30d live 53 traces 100% OK $0.02 cache 43%. Memo `BYOK_DECISION_20260918.md`. Canonical §9 queries verified.
...
### Changed
- **Provider-Isolated FIFO Conveyor Belt Architecture & Active Concurrency Tracking (`src/network/pacer.ts`, `src/handlers/`, `src/index.ts`)**:
  - **Strict Provider Isolation**: Refactored `RequestPacer` and `getPacerForProvider(providerCode)` into independent per-provider conveyor belts. Each provider maintains an isolated FIFO queue and state tracking, ensuring zero cross-provider queuing interference or contention.
  - **Single Entrance for Inbound & Retries**: Harmonized initial attempts and subsequent retry attempts into a single unified entrance; all requests acquire a slot on that provider's conveyor belt (`acquire(signal)`).
  - **Active In-Flight Stream Lease Lifecycle (`maxConcurrency`)**: Enhanced `RequestPacer` to return a `PacerAcquireResult` (`{ queueDwellMs, release }`). Implemented active concurrency tracking (`activeInFlight` bounded by `maxConcurrency` from `config/providers.json`). For streaming responses (`createStreamingResponse`, `wrapStreamWithPacerLease`), requests hold their concurrency lease until stream termination (EOF `done: true`, upstream error, or client disconnection/`cancel()`), enforcing real physical concurrency limits (e.g. Zen `max_concurrency: 1`).
  - **Elimination of Isolated Mini-Retries & Deadlock Prevention**: Retries no longer wait in isolated sleep loops holding concurrency slots. On retryable errors (429, 5xx), the failed attempt immediately invokes `pacerLease.release()` before sleeping for the jittered retry delay and re-queueing on the conveyor belt, eliminating head-of-line conveyor deadlocks.
  - **Elimination of Edge Double-Pacing**: Removed redundant gateway-edge ingress pacing (`acquireIngressPacer` in `src/index.ts`). Ingress requests flow directly to handler-level conveyor belts without duplicate delays.
  - **Eradication of Hardcoded `isZen` Checks**: Purged all remaining `isZen` branches, custom single-flight strategy hacks, and isolated loops from `src/handlers/openai_original.ts`, `src/handlers/openai_compat.ts`, and `src/engine/dispatch.ts`. Provider capabilities and concurrency are governed uniformly via `config/providers.json` (`strategy: standard`, `pacer.max_concurrency: 1`).
- **Zero-Hardcoding Configuration & Provider Operational Unification (`config/providers.json`, `src/config/`, `src/network/`, handlers)**:
  - **Single Source of Truth**: `config/providers.json` is now the sole source of truth across all active and catalog providers for operational parameters (`pacer`, `circuit_breaker`, `key_cooldown`, `request_retry`), completely replacing fragmented environment variables, magic numbers, and ad-hoc fallback values. All 13 providers explicitly declare these operational blocks.
  - **Verified Fail-Loud Boot Failure (`validateProviderConfigsFailLoud()`)**: Enforced hard startup abort (`process.exit(1)`) with `[FATAL] [ProviderRegistry] Provider configuration validation failed loudly refusing to start` when any provider entry in `config/providers.json` is missing required operational blocks (`pacer`, `circuit_breaker`, `key_cooldown`, `request_retry`). Verified via live tamper tests against `bun run src/index.ts` and `bash scripts/restart.sh`.
  - **Complete Deprecation Purge of Provider Environment Variables (`src/config/env.ts`, `src/config/schema.ts`)**: Formally purged all 17 legacy provider-specific operational environment variables (`GCP_ENABLE_RETRIES`, `GCP_MAX_RETRIES`, `GCP_RETRY_MIN_DELAY_MS`, `GCP_RETRY_MAX_DELAY_MS`, `GCP_ENABLE_QUARANTINE`, `GCP_ENABLE_CIRCUIT_BREAKER`, `ZEN_ENABLE_RETRIES`, `ZEN_MAX_RETRIES`, `ZEN_RETRY_MIN_DELAY_MS`, `ZEN_RETRY_MAX_DELAY_MS`, `ZEN_ENABLE_QUARANTINE`, `ZEN_ENABLE_CIRCUIT_BREAKER`, `ZEN_MIN_DELAY_MS`, `OPENROUTER_ENABLE_QUARANTINE`, `OPENROUTER_ENABLE_CIRCUIT_BREAKER`, `OPENROUTER_MIN_DELAY_MS`) from `DEFAULT_ENV_RECORD` and `EnvConfigSchema`.
  - **Elimination of Hardcoded Whitelists**: Eradicated static provider whitelists (`['or', 'nv', 'zn', 'gg']`) from `src/index.ts` (edge ingress pacer acquisition), `src/handlers/openai_compat.ts`, and `src/handlers/anthropic_compat.ts`. Ingress pacing and routing decisions are now driven generically by `isRegisteredProvider(provider)` and `provConfig.pacer.enabled`.
  - **Removal of Hardcoded Switch Branches**: Removed hardcoded `switch (provider)` branches from `src/network/pacer.ts` (`getPacerForProvider`) and `src/network/pool.ts` (`isProviderQuarantineEnabled`). Pacer intervals, queue depths, and dwell timeouts are read directly from `provConfig.pacer`, while quarantine eligibility respects `provConfig.key_cooldown.enabled` and `provConfig.circuit_breaker.enabled`.
  - **Decoupling of Cooldown Subsystem**: Decoupled `src/network/cooldown.ts` from the deprecated `COOLDOWN_RATE_LIMIT_TTL_SEC` environment variable. `CooldownManager` now accepts a configurable `defaultRateLimitTtlSec` parameter (defaulting to `RATE_LIMIT_DEFAULT_SEC = 65`), enabling clean dependency injection and hermetic, isolated testing without environment side-effects.
  - **Dynamic Endpoints & Error Classification in Responses API (`src/handlers/openai_original.ts`)**: Eliminated the static `UPSTREAM_URLS` map in favor of dynamic endpoint resolution via `config.endpoints["rs"]` (`resolveUpstreamResponsesUrl()`). Replaced Zen-only error classification with generic `classifyUpstreamError` across all upstream providers, enabling uniform key rotation, quarantine, and bounded jitter retry handling.
  - **Dynamic GCP Pacer Configuration (`src/handlers/gcp_compat.ts`)**: Decoupled GCP Vertex AI / Gemma ingress pacing from static environment variables, sourcing `min_delay_ms`, `max_queue_wait_ms`, and pacer enablement directly from `provConfig.pacer`.
  - **Zen Single-Flight Policy**: Configured the Zen provider (`zn`) explicitly in `config/providers.json` for single-flight execution (`max_attempts: 1`, `delay: { min_ms: 0, max_ms: 0 }`), eliminating upstream OpenCode session burning and `429 FreeUsageLimitError` without hardcoded loop branches.
  - **Dynamic Environment Key Resolution & Memory Efficiency**: Dynamically resolved `env_key` in `src/config/keys.ts` via `getProviderConfig(provider).env_key` with fallback to `PROVIDER_ENV_MAP`. Completely eliminated redundant in-memory file parsing (`getProvidersRegistry`, `cachedRegistry`) in `src/handlers/openai_compat.ts`, wiring directly to central `initProviderRegistry()` and `getProviderConfig()`.
  ...
  - **Comprehensive Test Suite Alignment**: Aligned all unit and integration suites with dynamic provider configuration, achieving 100% pass across 1,039 gateway tests and 1,281 total tests.
