...
### Changed
- **Zero-Hardcoding Configuration & Provider Operational Unification (`config/providers.json`, `src/network/`, handlers)**:
  ...
  - **Strict Boot-Time Schema Validation**: `ProviderConfigEntrySchema` strictly requires all four operational blocks (`pacer`, `circuit_breaker`, `key_cooldown`, `request_retry`) on every registered provider. Missing blocks or invalid configurations fail loudly during boot with formatted diagnostic logs (`[FATAL] [ProviderRegistry] Provider configuration validation failed loudly refusing to start`) and abort gateway startup (`process.exit(1)`).
