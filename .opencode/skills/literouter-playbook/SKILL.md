...
3. **Google Provider Ghosting:**
   Google provider also applies first-byte ghosting via `upstreamSignal` in `src/config/env.ts`. The same 5s timeout and 1s retry delay apply.
4. **Exhaustion Guard:**
