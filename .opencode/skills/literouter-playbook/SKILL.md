...
| **`src/transformers/payload.ts`** | Manages thought signatures (`injectThoughtSignature`, `extractThoughtSignature`), token estimation (`estimateTokens`), Gemma unsupported field stripping (`cleanGemmaPayload`), LaTeX formatting (`cleanLatexSymbols`), message merging (`mergeConsecutiveMessages`), historical message sanitization (`sanitizeHistoricalMessages`), non-streaming reasoning transformations (`transformNonStreaming`), and streaming SSE stream transformations (`createStreamTransformer`). Exports `StreamMeta` interface. The `createStreamTransformer` function transforms upstream SSE chunks for OpenAI-compatible providers, streaming reasoning chunks (`delta.reasoning`, `delta.reasoning_content`) in real-time enclosed inside `<thought>\n` ... `\n</thought>\n` tags directly in `delta.content` when `collapseReasoning=true`. This enables native collapsible thinking UI in clients like OpenCode without discarding tokens or forcing manual continue prompts. Closing `\n</thought>\n` tags are enqueued in a dedicated, isolated SSE chunk to prevent corrupting downstream `tool_calls` structures when models transition directly from reasoning to tool execution. Handles multi-phase reasoning streams (`!hasStartedThought || hasEndedThought` state reset) to ensure interleaved thinking phases are cleanly enclosed in `<thought>` tags. A keep-alive timer injects SSE comment lines (`:\n\n`) every 2 seconds to prevent downstream SSE clients from idling out during long reasoning streams. |
