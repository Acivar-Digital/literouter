...
describe("GCP Retry Toggle & Resilience Handler (GCP_ENABLE_RETRIES & GCP_ENABLE_QUARANTINE)", () => {
  ...
  it("3. Key quarantine preservation: records failure for key 0 so subsequent request picks key 1", async () => {
    ...
    resetAllState();
    getProviderConfig("gc").request_retry.enabled = false;
    if (getProviderConfig("gc").key_cooldown) getProviderConfig("gc").key_cooldown!.enabled = true;
  ...
  it("7. Dumb forwarder mode (GCP_ENABLE_QUARANTINE=false): fails key on 429 without placing key into quarantine", async () => {
    ...
    resetEnvCache();
    resetAllState();
    const gc7 = getProviderConfig("gc");
    if (gc7?.key_cooldown) gc7.key_cooldown.enabled = false;
  ...
  it("8. Combined dumb forwarder (GCP_ENABLE_RETRIES=false + GCP_ENABLE_QUARANTINE=false): transparent pass-through without lockout", async () => {
    ...
    resetEnvCache();
    resetAllState();
    const gc8 = getProviderConfig("gc");
    if (gc8?.request_retry) gc8.request_retry.enabled = false;
    if (gc8?.key_cooldown) gc8.key_cooldown.enabled = false;
  ...
  it("9. KeyPool provider isolation: GCP_ENABLE_QUARANTINE=false only disables quarantine for gc, not other providers", async () => {
    ...
    resetEnvCache();
    resetAllState();
    const gc9 = getProviderConfig("gc");
    if (gc9?.key_cooldown) gc9.key_cooldown.enabled = false;

    globalKeyPool.setPool("gc", ["mock-gc-key"]);
    globalKeyPool.setPool("nv", ["mock-nv-key"]);

    // Report failure for GCP
    globalKeyPool.reportFailure("gc", 0, 429, { "retry-after": "60" });
    ...
    globalKeyPool.reportFailure("nv", 0, 429, { "retry-after": "60" });
  ...
  it("10. Circuit breaker isolation (GCP_ENABLE_CIRCUIT_BREAKER=false): 5 consecutive 503s do not trip breaker or block subsequent requests", async () => {
    ...
    resetEnvCache();
    resetAllState();
    const gc10 = getProviderConfig("gc");
    if (gc10?.request_retry) gc10.request_retry.enabled = false;
    if (gc10?.key_cooldown) gc10.key_cooldown.enabled = false;
