...
- **Fix**: See single source of truth `directive-grammar.md` §1 (direct `lr-<provider>-<payload>-<completion>-<nuance>` vs fusion `lr-fse-<preset>`; presets on disk: `quad`, `pydn`, `fast`, `deep`).
...
- **Root Cause & Forensic Breakdown**:
  1. **HTTP/2 Multiplexing Blast Radius**: Outbound upstream requests to providers (e.g. `https://openrouter.ai`) share persistent HTTP/2 TCP sockets (`Http2Session` in `src/network/h2_pool.ts`). When an upstream load balancer resets or drops an overloaded connection (`RST_STREAM`/`GOAWAY`), Node/Bun's `http2` runtime terminates *all multiplexed in-flight streams* on that socket simultaneously with `The pending stream has been canceled`.
  2. **Little's Law Mismatch ($L = \lambda \times W$)**: RPM limits (e.g. 20 RPM/key $\times$ 15 keys = 300 RPM) describe completion throughput ($\lambda$), but connection load ($L$) is governed by request duration ($W$). For high-reasoning free models (e.g. `minimax/minimax-m3:free` with $\text{TTFT} \approx 27\text{s}$ and total latency $W \approx 60\text{s}$), running 45 concurrent workers creates 45 simultaneous in-flight streams ($45 / 15 = 3\text{ streams/key}$), violating upstream per-key/per-IP free-tier concurrency caps ($\le 1-2$ active streams).
- **Resolution & Sizing Heuristics**:
