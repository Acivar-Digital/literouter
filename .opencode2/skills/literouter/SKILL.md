...
- **Zero-Hardcoding Configuration & Dynamic Provider Binding (`config/providers.json`)**: All retry attempts, jitter delays, pacers, and circuit breakers across all four handlers (`openai_compat.ts`, `anthropic_compat.ts`, `openai_original.ts`, `gcp_compat.ts`) are driven dynamically by operational parameters in `config/providers.json` (`provConfig.request_retry`, `provConfig.pacer`, `provConfig.circuit_breaker`), eradicating legacy hardcoded magic numbers (`Math.min(3, ...)`). The Zen provider (`zn`) is explicitly configured for single-flight execution (`max_attempts: 1`, `delay: { min_ms: 0, max_ms: 0 }`) to eliminate session burning and `429 FreeUsageLimitError`.
