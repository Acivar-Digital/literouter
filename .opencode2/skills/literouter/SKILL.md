...
- **Zero-Hardcoding Configuration & Provider Operational Unification (`config/providers.json`)**: `config/providers.json` is the sole source of truth for all provider operational parameters. All four operational blocks (`provConfig.request_retry`, `provConfig.pacer`, `provConfig.circuit_breaker`, `provConfig.key_cooldown`) are strictly required by `ProviderConfigEntrySchema` on every registered provider; missing fields fail loudly and abort gateway startup. All retry attempts, bounded jitter delays, conveyor pacers, and circuit breakers across all four handlers (`openai_compat.ts`, `anthropic_compat.ts`, `openai_original.ts`, `gcp_compat.ts`) and network subsystems are driven dynamically, eradicating hardcoded whitelists (`['or', 'nv', 'zn', 'gg']`), switch statements, and magic numbers (`Math.min(3, ...)`). The Zen provider (`zn`) is explicitly configured for single-flight execution (`max_attempts: 1`, `delay: { min_ms: 0, max_ms: 0 }`) to eliminate session burning and `429 FreeUsageLimitError`.
