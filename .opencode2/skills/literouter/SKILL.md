...
22. **OpenRouter Agentic Harness Whitelist Headers**: Automatically injects approved agentic harness headers (`HTTP-Referer`, `X-Title`, `User-Agent` defaulting to OpenCode, env-configurable) when calling OpenRouter, preventing HTTP 403 `Gate Free Endpoints by Agentic Harness` errors on `:free` models. Details in `openrouter-handling-spec.md`.
23. **Zen Provider OpenCode Identity Gating (`literouter-oltw`)**: OpenCode's Zen gateway (`opencode.ai/zen/v1`) gates API access behind `User-Agent: OpenCode` and OpenCode Referer headers (returning synthetic `HTTP 429 FreeUsageLimitError: Rate limit exceeded` when called with standard runtime User-Agents like Bun, curl, or python-requests). `buildAuthHeaders` and `probeZenKey` automatically inject `User-Agent: OpenCode/1.0.0` and `HTTP-Referer`/`Referer: https://opencode.ai`.
