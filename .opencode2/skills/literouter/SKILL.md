...
- **Zero-Hardcoding Configuration & Provider Operational Unification (`config/providers.json`)**: `config/providers.json` is the sole source of truth for all provider operational parameters. All four operational blocks (`provConfig.request_retry`, `provConfig.pacer`, `provConfig.circuit_breaker`, `provConfig.key_cooldown`) are strictly required by `ProviderConfigEntrySchema` on every registered provider; missing fields fail loudly and abort gateway startup. All retry attempts, bounded jitter delays, conveyor pacers, and circuit breakers across all handlers (`openai_compat.ts`, `anthropic_compat.ts`, `openai_original.ts`, `gcp_compat.ts`, `dispatch.ts`) and network subsystems are driven dynamically, eradicating hardcoded whitelists (`['or', 'nv', 'zn', 'gg']`), switch statements, magic numbers (`Math.min(3, ...)`), and hardcoded `isZen` checks. Concurrency and single-flight execution are managed natively via `provConfig.pacer.max_concurrency` (e.g. `zn` configured with `max_concurrency: 1`) rather than isolated handler loops or hardcoded flags.
...
- **Provider-Isolated FIFO Conveyor Belt Architecture (`src/network/pacer.ts`)**:
  - **Strict Provider Isolation**: Each provider maintains its own independent FIFO `RequestPacer` conveyor belt instance managed via `getPacerForProvider(providerCode)` in a global pacer registry (`pacerRegistry`). Cross-provider contention is strictly isolated (e.g. Zen requests never block OpenRouter or Google requests).
  - **Single Entrance for Both Initial Requests and Retries**: All attempts acquire a slot on that provider's conveyor belt (`await pacer.acquire(signal)`). Whether an inbound request is on its first attempt or a subsequent retry, it enters the same FIFO pipeline.
  - **NO Isolated Mini-Retries & Deadlock Prevention**: Retries do not sit in disconnected sleep loops that bypass queue ordering. When an attempt encounters a retryable failure (e.g. 429 or 5xx), it **immediately releases its concurrency lease** (`pacerLease.release()`) before sleeping for the retry backoff delay. This prevents conveyor head-of-line deadlock where sleeping retries hold in-flight slots and starve queued requests.
  ...
  - **Zero Hardcoded `isZen` Checks**: All handlers dynamically query provider capabilities via `getProviderConfig(provider)`. Hardcoded `isZen` checks, branch exceptions, and provider-specific retry logic have been eradicated across all handlers (`openai_compat.ts`, `openai_original.ts`, `anthropic_compat.ts`, `dispatch.ts`).
...
| 2 | Error classification & key rotation (`classifyUpstreamError`) | In-flight retry driven by `providers.json` (`provConfig.request_retry.max_attempts`, ≤poolSize); 429 dynamic / 5xx 10s / 401-403 tiered / fail-fast 400-404 | [error-action-matrix.md §1](error-action-matrix.md#1-status-action-matrix) |
...
| 12 | Provider-isolated FIFO conveyor belt & active concurrency pacer (`pacer.ts`) | Strict per-provider isolation (`getPacerForProvider`), single entrance for initial/retries, active in-flight stream lease tracking (`max_concurrency`), zero edge double-pacing | [error-action-matrix.md §4](error-action-matrix.md#4-env-knob-quick-reference) |
