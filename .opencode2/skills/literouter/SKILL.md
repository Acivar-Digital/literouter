...
- **Google Native Dumb Forwarder (`@ai-sdk/google`)**: Use `lr-gg-gg-gc-no` (payload: `gg`, endpoint: `gc`). Routes native Gemini REST requests (`/v1beta/models/*:streamGenerateContent?alt=sse` and `/v1beta/models/*:generateContent`) directly to `generativelanguage.googleapis.com`. Supports `@ai-sdk/google` or standard Gemini SDKs by configuring `baseURL: "http://localhost:7766/v1beta"` with `apiKey: "lr-gg-gg-gc-no"` (or via query `?key=lr-gg-gg-gc-no`). Features transparent byte-stream passthrough, strips client `authorization`, injects rotated upstream keys into `x-goog-api-key`, strips downstream hop-by-hop/compression headers (`content-encoding`, `transfer-encoding`), and retries in-flight on 429/5xx across `GOOGLE_API_KEYS`. See `google-native.md` for complete architecture, HTTP/2 connection pooling, OpenCode2 Free Tier key rotation, and diagnostic matrix.
