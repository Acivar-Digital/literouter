...
| Unit tests (all) | `bun test` or `bun test:lr` (accelerated domain-partitioned test runner: runs 7 domains in parallel subprocesses, completely silent on success, outputs only isolated failures) |
...
> **Accelerated Test Runner & Subcommands (`bun test` / `bun test:lr`)**:
> - `bun test` or `bun test:lr`: Accelerated domain-partitioned test runner (runs 7 domains in parallel subprocesses, completely silent on success, outputs only isolated failures)
> - `bun test <domain>`: Fast targeted slice (e.g. `bun test handlers`, `bun test network`, `bun test stream`, `bun test engine`, `bun test telemetry`, `bun test core`, `bun test legacy`, `bun test eval`)
> - `bun run test:raw`: Raw unbuffered Bun test runner (verbose escape hatch)
> - OpenCode2 tool: `test_literouter` native plugin tool for zero-bloat programmatic test execution.
...
- **Purged Dead Ballast**:
  ...
  - `circuit_breaker` is retired across all providers and made safe pass-through (zero artificial 503 outage tripping).
  - Fine-grained `key_cooldown` knobs and `max_delay_ms` are purged.
...
- **Google Native (gg) v4 Path Model Extraction, Stream Action Preservation & Query Passthrough**: In Engine v4 (`src/handlers/v4/google_native.ts`, `src/engine/strategies/native_cascade.ts`), inbound requests to `/v1beta/models/*` (`gc`) or `/v1/models/*` (`g1`) automatically extract the model identifier from the URL pathname (`/\/(?:v1beta|v1)\/models\/([^:]+)/`) when missing from the body. When the request targets `:streamGenerateContent`, `NativeCascadeStrategy.buildGoogleUrl` preserves the `:streamGenerateContent` RPC action instead of falling back to `:generateContent`. Furthermore, query parameters (such as `?alt=sse`, excluding client-side `?key=...` auth tokens which are converted to rotated headers) are preserved on the upstream request, and streamed responses return over SSE (`text/event-stream`).
...
| **Circuit Breaker** | `config/providers.json` -> `circuit_breaker` | Retired pass-through across all providers (zero artificial 503 outage tripping). |
| **Provider Operational Knobs** | `config/providers.json` (`pacer`, `circuit_breaker`, `key_cooldown`, `request_retry`) | Sole source of truth across all 13 providers. Handlers look up settings directly with zero shadow defaults or magic constants. All 17 legacy provider env vars purged. |
...
- **Single FIFO Conveyor Pipe (`src/network/pacer.ts`)**:
  ...
  - **Strict Spacing by `min_delay_ms`**: All inbound requests and retries share the same conveyor belt, spaced strictly by `min_delay_ms` (`await pacer.acquire(signal)`).
  ...
  - **NO Isolated Mini-Retries & Deadlock Prevention**: Retries do not sit in disconnected sleep loops that bypass queue ordering. When an attempt encounters a retryable failure (e.g. 429 or 5xx), it **immediately releases its concurrency lease** (`pacerLease.release()`) before sleeping for the retry backoff delay, preventing conveyor head-of-line deadlock.
...
| 13 | Retired circuit breaker (safe pass-through) | Retired across all providers and made safe pass-through; zero artificial 503 outage tripping | [error-action-matrix.md §2](error-action-matrix.md#2-retry-quarantine-call-chain-wiring) |
...
| 28-30 | Zen operational knobs (`config/providers.json`) | Governed via `zn` operational blocks (`strategy: standard`, `max_concurrency: 1`); legacy env toggles and hardcoded `isZen` checks purged | [config-schemas.md §3.6](config-schemas.md#36-operational-governance--zero-hardcoding-request_retry-key_cooldown-pacer-circuit_breaker) |
...
| Provider operational knobs (`request_retry`, `key_cooldown`, `pacer`, `circuit_breaker` pass-through) & boot validation (`validateProviderConfigsFailLoud()`) | [config-schemas.md §3.6](config-schemas.md#36-operational-governance--zero-hardcoding-request_retry-key_cooldown-pacer-circuit_breaker) |
...
12. **Retired Circuit Breaker Safe Pass-Through**: `circuit_breaker` is retired across all providers and made safe pass-through, eliminating artificial 503 outage tripping while maintaining configuration schema compatibility.
