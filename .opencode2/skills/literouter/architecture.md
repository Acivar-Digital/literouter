...
A frequent architectural inquiry when evaluating an enterprise API gateway is: *"Why does LiteRouter not use Redis, Valkey, or Bun's native `Bun.redis` (`Bun.RedisClient`) for rate limiting and state coordination?"*
...
This architectural posture is rooted in deliberate systems engineering tradeoffs, not omission:

1. **Event-Loop Non-Preemptive Atomicity**:
...
4. **Zero External Failure Domains**:
   Eliminates daemon dependencies, connection pool exhaustion, socket timeouts, and boot gating on external services. If Valkey/Redis crashes or stalls, an external-dependent gateway goes dark. LiteRouter boots instantaneously with zero external dependencies, zero socket leaks from orphaned client connections, and zero configuration friction.

5. **Proper Scope vs. Clustering**:
   If multi-node horizontal clustering across multiple physical machines is needed in the future, a pluggable adapter can be introduced. But for a high-performance, single-instance gateway, forcing an external Valkey daemon is unnecessary operational baggage. Single-instance throughput on Bun easily handles tens of thousands of concurrent requests without external coordination overhead.
...
> Engine gate first: `resolveEngine(req)` (`src/config/env.ts:130-140`) branches at `src/index.ts:414-417` — `v4` goes to `dispatchV4` (`src/handlers/v4/router.ts:201-211`), otherwise the legacy order below applies. Default engine is `legacy` (`src/config/env.ts:51`, `src/config/schema.ts:199`). `/v1/traces` is v4-only (`router.ts:93-95`); on legacy the path falls through legacy routing to `404`. Full gate: `directive-grammar.md` §11; env knobs: `config-schemas.md` §7.
...
> `strategy` (default `"standard"`, `src/config/schema.ts:113-133`) selects the per-provider execution engine — `google` → `native_cascade`, `zen` → `zen_single_flight`, `gcp` → `gcp_guarded` (`config/providers.json:112,338,400`). An unknown `strategy` throws at boot (`src/engine/strategy_registry.ts:62-64`) and `src/index.ts:49-57` exits 1 — a bad strategy is fatal, never fail-open. Canon: `config-schemas.md` §3.5.
