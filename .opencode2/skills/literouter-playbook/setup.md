...
| `LITEROUTER_STREAM_IDLE_TIMEOUT` | `30` | `number` | `parseInt(Bun.env.LITEROUTER_STREAM_IDLE_TIMEOUT \|\| Bun.env.LITEROUTER_STREAM_IDLE_TIMEOUT_MS \|\| "30", 10) * 1000`<br>Inter-chunk stream idle timeout in **seconds** (converted to milliseconds: `30,000ms`). Max time allowed between streamed tokens/reasoning chunks before assuming stream is stalled. |
