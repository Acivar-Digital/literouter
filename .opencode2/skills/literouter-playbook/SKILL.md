...
10. **Outbound HTTP/2 Multiplexed Session Pool (`src/network/h2_pool.ts`)**: Coalesces concurrent outbound requests into persistent HTTP/2 sessions with single-flight mutexes, in-pool `GOAWAY` stream tracking, zero-stream auto-teardown, and permanent runtime error lifecycle handlers that immediately purge and destroy zombie/reset sockets (`ECONNRESET`, `EPIPE`) to prevent key failover loops from reusing dead connections. Falls back to HTTP/1.1 keep-alive on failure.
