...
1. **TTFT Guard** (5s): aborts upstream if no verifiable content token arrives, rotates to next key with zero cooldown penalty.
2. **Stream Idle Guard** (30s) & **Bun Idle Timeout** (60s): protects against mid-stream stalls while giving slow/reasoning models sufficient inter-chunk leeway.
3. **SSE Keepalive** (2s/15s): injects comment frames (`: keep-alive\n\n`) to keep client connections active during thinking and long processing pauses.
4. **Ghost Response Guard**: rejects HTTP 200 responses with 0 content tokens.
5. **Client Cache Sanitizer**: strips `prompt_cache_key`/`prompt_cache_retrieval`/`prompt_cache_reset` before upstream dispatch.