...
11. **Outbound HTTP/2 Multiplexed Session Pool (`src/network/h2_pool.ts`)**: Coalesces concurrent outbound requests into persistent HTTP/2 sessions with single-flight mutexes, in-pool `GOAWAY` stream tracking, zero-stream auto-teardown, proactive socket liveness verification (`isSessionHealthy`), and permanent runtime error lifecycle handlers (`purgeSession`) that immediately purge and destroy zombie/reset sockets (`ECONNRESET`, `EPIPE`, idle timeouts) to prevent key failover loops from reusing dead connections. Falls back to HTTP/1.1 keep-alive on failure.
12. **Token-Bucket Rate Pacer & Ingress Conveyor Belt (`src/network/pacer.ts`)**: Enforces mandatory `minIntervalMs` (2000ms for Google `gg`, 500ms for others) with an $O(1)$ fast queue, 300s bounded queue dwell timeout, and unified conveyor belt pacing for both inbound client requests and mid-stream retries.
